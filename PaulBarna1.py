...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if len(OTHER) == 1:
        ...
        y = OTHER[0][1]
        xy_estimate = (x, y)
    #if the length of the measurement list is 2, distance and heading become 0 
    elif len(OTHER) == 2:
        distance= [0]
        heading= [0]
    else:
        # otherwise calculate the distance and the heading by referring to the previous measurement values
        for i in range(len(OTHER)-1):
            Initial = (OTHER[i][0], OTHER[i][1])
            Next = (OTHER[i+1][0], OTHER[i+1][1])
            distance.append(distance_between(Next, Initial))
            heading.append(atan2(OTHER[i+1][1]-OTHER[i][1], OTHER[i+1][0]-OTHER[i][0]))
    
    turningPredict=0
    
    #if the length of the heading list is greater than 1, turning is calculated based on the heading values
    if len(heading)> 1:  
        for j in range(len(heading)-1):
                turning.append((heading[j+1] - heading[j])%(2*pi))
        turningPredict = sum(turning) / float(len(turning))
        
    #if the length of the measurement list is not 1, the x and y estimates are finally calculated
    if len(OTHER) !=1:
        xPrevious = OTHER[-1][0]
        yPrevious = OTHER[-1][1]
        distancePredict = sum(distance) / float(len(distance))
        xPredict = xPrevious + distancePredict * cos(heading[len(heading)-1] + turningPredict)
        yPredict = yPrevious + distancePredict * sin(heading[len(heading)-1] + turningPredict)
